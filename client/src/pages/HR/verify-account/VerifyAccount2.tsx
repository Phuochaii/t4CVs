...
function VerifyAccount2() {
  ...
  const handleUploadLicense = async () => {
    ...
    // });
    await axios
      .put('http://34.28.130.105/employer/update/license', formData)
      .then((res) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://34.28.130.105/employer/1');
      ...
      <div className="text-black mt-12 mx-[200px] mb-20 px-8 pt-8 pb-10 bg-white rounded">
        <div className="flex gap-6 pb-4 border-b border-[#e8edf2]">
          ...
          <div>
            ...
            <div className="font-base">
              Thực hiện xác minh thông tin để đảm bảo thông tin chính chủ và
              tăng cường bảo mật
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 flex justify-between items-center">
          <div className="flex items-center w-[65%]">
            ...
            />
            <span>
              Quét mã QR để thực hiện xác minh thông tin trên điện thoại
            </span>
          </div>
          <button
            onClick={() => {
