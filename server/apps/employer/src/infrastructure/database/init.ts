...
export class DatabaseConfiger {
  ...
  private async insertData() {
    const option = {
      ...this.defaultConfig,
      entities: [EmployerSchema, PositionSchema],
    } as DataSourceOptions;
    await doCallbackWithAutoCloseConnection(option, async (dataSource) => {
      await dataSource.getRepository(PositionSchema).insert(positions);
      await dataSource.getRepository(EmployerSchema).insert(employers);
    });
  }
